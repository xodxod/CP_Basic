...
int main()
{
	...
	*/
	...
	//ClothColor == 'B'
	/*
	if (TallNumber >= 189 && ClothColor == 'B') {
		cout << "범인" << endl;
	}
	else {
		cout << "시민" << endl;
	...
	*/
	...
	for (int i = 0; i < SortNumber; i++) {
		...
		cin >> ProductNumber;
		//3-3
		PruductToralPrice = ProductPrice * ProductNumber;
		//3-4
		SumTotalPrice = SumTotalPrice + PruductToralPrice;
	}
	...
	*/
	/*
	* 총금액 입력 받고 각물건의 개수*금액 총합이 총금액과 같아질 때 까지 반복실행해서 같은지 비교
		1.총금액 입력
	...
	*/
	...
	while (TotalPrice1 != 0) {
		...
		cin >> ProductNumber1;
		//2-3
		PruductTotalPrice1 = ProductPrice1 * ProductNumber1;
		//2-4
		TotalPrice1 = TotalPrice1 - PruductTotalPrice1;
	}
	if (Zero == false) {
		cout << "No" << endl;
	}
